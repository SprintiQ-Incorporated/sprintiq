...
-- Create a simple index for vector similarity search (fallback if HNSW is not available)
-- Using ivfflat with fewer lists for better compatibility
CREATE INDEX IF NOT EXISTS tawos_user_stories_embedding_idx 
ON tawos_user_stories 
USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 50);

-- Alternative: If ivfflat still fails, use no index (slower but will work)
-- CREATE INDEX IF NOT EXISTS tawos_user_stories_embedding_idx 
-- ON tawos_user_stories 
-- USING btree (id);
